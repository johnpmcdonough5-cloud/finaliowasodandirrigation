...
Each run this picks one, renders it into the site's own chrome (cloned from an
...
Two failure modes this deliberately does NOT hide (a quiet, always-green
workflow is what let this blog's predecessor run for five weeks publishing
nothing):
  - an empty queue exits 1, not 0
  - a stale blogbot/published/ (newest post older than STALE_DAYS) exits 1
"""
...
def check_freshness(today: dt.date, name: str) -> None:
    ...
    if newest is None:
        return  # nothing published yet — first run, nothing stale to report
